...
class MoviesDataset(BaseDataset):
    ...
    def _generate_synthetic(self):
        ...
        for i, (actor, movie) in enumerate(facts):
             # The template asks "Who acted as {person}..." -> wait, "Who acted as [figure] in [movie]?" 
             # usually implies Answer=Figure.
             # Wait, the prompt in ACT-ViT/Orgad is: "Who acted as [figure] in [movie]?"
             # That sounds like "Who played the role of [Character]?" 
             # Or is it "Who acted as [Actor]?" -> No, that's tautological.
             # Let's re-read carefully: "Who acted as **[figure]** in the movie **[movie]**?"
             # If [figure] is the Character Name, then Answer is Actor.
             # If [figure] is Actor Name, then Answer is Character? 
             # "Who acted as Neo in the movie The Matrix?" -> Keanu Reeves. 
             # That makes sense.
             
             # So the facts should be (Character, Movie, Actor)
             # Let's fix the synthetic data.
